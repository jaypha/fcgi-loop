...
/*
 * This is a D port of fcgiapp.h by Open Market.
 *
 * Copyright and license are inherited from fcgiapp.h.
 * Port done by: Jason den Dulk
 */

module jaypha.fcgi.c.fcgiapp;
...
const int FCGX_UNSUPPORTED_VERSION  = -2;
const int FCGX_PROTOCOL_ERROR  = -3;
const int FCGX_PARAMS_ERROR = -4;
const int FCGX_CALL_SEQ_ERROR  = -5;
...
struct FCGX_Stream {
    ubyte *rdNext;    /* reader: first valid byte
                               * writer: equals stop */
    ubyte *wrNext;    /* writer: first free byte
                               * reader: equals stop */
    ubyte *stop;      /* reader: last valid byte + 1
                               * writer: last free byte + 1 */
    ubyte *stopUnget; /* reader: first byte of current buffer
                               * fragment, for ungetc
                               * writer: undefined */
    int isReader;
    int isClosed;
...
struct FCGX_Request {
    ...
    void* paramsPtr;          /* Params? WTF?  - Params* paramsPtr; */
